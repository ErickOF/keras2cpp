...
void keras::DataChunk2D::flatten()
{
  ...
  for (std::vector<vector<float>> &v1: data)
...
{
  cout << "Loading data chunk2d from " << fname << endl;
...
void keras::LayerConv2D::load_weights(std::ifstream &fin)
{
  ...
  if (this->verbose)
    cout << "LayerConv2D " << m_kernels_cnt << "x" << m_depth << "x"
      << m_rows << "x" << m_cols << " border_mode " << m_border_mode << endl;
...
void keras::LayerDense::load_weights(std::ifstream &fin)
{
  ...
  if (this->verbose)
    cout << "Inputs: " << m_input_cnt << ", neurons: " << m_neurons << endl;
    ...
    std::vector<std::vector<float>> const &k)
{
  ...
  cout << "Kernel: " << k1_size << "x" << k2_size << endl;
...
{
  if (this->verbose)
    std::cout << "Running: " << this->get_name() << endl;
  ...
  if (this->verbose) {
    cout << "y_ret: " << y_ret.size() << "," << y_ret[0].size() << "," << y_ret[0][0].size() << endl;
    cout << "kernel: " << m_kernels.size() << "x" << m_kernels[0].size() << "x" << m_kernels[0][0].size() << "x" << m_kernels[0][0][0].size() << endl;
    cout << "img: " << im.size() << "x" << im[0].size() << "x" << im[0][0].size() << endl;
  }
  ...
  { // loop over kernels
    ...
    { // loop over image depth
      vector<vector<float>> tmp_w = (m_border_mode == "valid") ? keras::conv_single_depth_valid(im[m], m_kernels[j][m]) : keras::conv_single_depth_same(im[m], m_kernels[j][m]);
