...
print('There are', str(len(model.layers)),
      'layers in your network, is it good?')
print('I think yes :)')


first_layer = arch['config']['layers'][0]['config']
...
random_input = random_input.reshape((-1, input_shape[3], input_shape[1], input_shape[2]))

print('Random input shape for keras2cpp', random_input.shape)
# store one sample in text file
# this code is working for input_shape[1] == 1
if random_input.shape[1] != 1:
    print('-'*50)
...
with open(args.data_sample, 'w') as fin:
    print(f'Save to {args.data_sample} sample shape {input_shape[3]} {input_shape[1]} {input_shape[2]}')

    fin.write(f'{input_shape[3]} {input_shape[1]} {input_shape[2]}\n')
